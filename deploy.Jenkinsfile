...
pipeline {
    ...
    stages {
        ...
        stage ('Déploiement des services') {
            steps {
                sh "cd sx5-services && docker login -u admin -p admin123 https://nexus3.inspq.qc.ca:5000"
                sh "ansible-playbook sx5-services/deploy.yml -i ${WORKSPACE}/sx5-services/LOCAL/LOCAL.hosts"
            }
        }
        stage ('Déploiement du UI') {
            steps {
                sh "cd sx5-ui && docker login -u admin -p admin123 https://nexus3.inspq.qc.ca:5000"
